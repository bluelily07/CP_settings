*cf handle: bluelily07*

## Динамическое Программирование

ДП - это способ решать задачи, разбивая их на **подзадачи**, которые повторяются. Вместо того чтобы каждый раз пересчитывать одну и ту же подзадачу заново (как в рекурсии), мы один раз считаем результат и **запоминаем** его. Полученный ответ используем в решении следующей подзадачи, не пересчитывая все с нуля.

## Виды ДП

Одномерное ДП - когда состояние описывается одним числом (индексом), и `dp[i]` хранит ответ для этого индекса. Это самый базовый и самый частый вид ДП. 

То есть у ДП есть только один хранимый параметр, у двухмерного ДП - два параметра, у трехмерного - три и т.д

## Способы реализации одномерного ДП

Примеры реализаций будут написаны для чисел Фибоначчи.

**Формула чисел Фибоначчи** - ```fib[i] = fib[i-1] + fib[i-2];```
где ```fib[0] = 0; fib[1] = 1;```

1. **Мемоизация**

Мемоизация - рекурсия с запоминанием результата. Проверяем, считали ли мы это состояние раньше? Если да - берем ответ из памяти, иначе считаем и записываем в память 

```cpp
vector<ll> mem;
vector<bool> visited;

ll solve(int n) {
    if (n <= 1) return n;//базовый случай рекурсии (fib0 = 0, fib1 = 1)
    if (visited[n]) return mem[n];//Если мы уже считали это, то просто берем из памяти, не считая заново.
    visited[n] = true;//иначе ставим флаг посещенности 
    mem[n] = solve(n - 1) + solve(n - 2);// и рекурсивно считаем 
    return mem[n];
}
```
2. **Табуляция**

Никакой рекурсии, считаем ДП от малых значений к большим, используя уже известные данные

```cpp
vector<ll> dp(n + 1)
dp[0] = 0;
if (n >= 1) dp[1] = 1;//создаем вручную два базовых значения
for (int i = 2; i <= n; i++) {
    dp[i] = dp[i - 1] + dp[i - 2];//заполняем ДП без рекурсии на основе известных значений
}
```

**Пару слов об реализациях:**
1. Чаще всего на практике - мемоизацию легче придумать.
2. Мемоизация дороже табуляции из-за доп. вызовов функций, дороже по памяти
3. При глубокой рекурсии мемоизация может словить stack overflow
4. Табуляция часто работает быстрее мемоизации
5. При табуляции можно не хранить весь массив, а только необходимые элементы для подсчета ДП.
6. При табуляции легче контролировать память 

## Разбор задач на одномерное ДП

Существует прямая пронумерованная от 0 до n. Стоя на позиции i, можно прыгнуть на позиции i+1 или i+2 или i+k. (k <= n, k != 1, k != 2).
Сколько существует различных последовательностей прыжков, чтобы добраться из позиции 0 в позицию n?

**Решение:**
Для начала, обозначим состояние ДП и базовое состояние.

`dp[i]` - количество различных последовательностей прыжков, чтобы попасть из позиции 0 в позицию i. - это наше состояние ДП.


`dp[0] = 1` - т.к, мы уже стоим на позиции 0, способов попасть на эту позиции всего один. Мы же начинаем с него. - Это наше базовое состояние.

Теперь мы должны обозначить переход состояний.

Чтобы оказаться в позиции i, последний прыжок был длиной 1, 2 или k. Значит, мы попали в i либо из `i-1`, либо из `i-2`, либо из `i-k`. Эти три случая не пересекаются между собой (у путей разная длина последнего прыжка — значит, это разные последовательности), и вместе покрывают все способы добраться до i. Поэтому их количества складываются:

```
dp[i] = dp[i-1] + dp[i-2] + dp[i-k]
```
При этом, важно чтобы позиция которую мы используем существовала, то есть была >= 0. - Это и есть переход состояний.

```cpp
void solve() {
    int n, k;
    cin >> n >> k;

    vector<ll> dp(n + 1, 0);
    dp[0] = 1;//базовое состояние

    for (int i = 1; i <= n; i++) {
        if (i - 1 >= 0) dp[i] = (dp[i] + dp[i - 1]);//первый способ попасть в позицию i, всегда true, тк у нас есть базовое состояние.
        if (i - 2 >= 0) dp[i] = (dp[i] + dp[i - 2]);//второй способ попасть в позицию i, если i>=2, переход возможен.
        if (i - k >= 0) dp[i] = (dp[i] + dp[i - k]);//третий способ попасть в позицию i.
    }

    cout << dp[n] << "\n";//последний элемент построенного массива равен кол-ву всех возможных последовательностей прыжков в n. 
}
```

**Разборы задач я вынес в отдельный файл SolvingDP.md, чтобы не флудить в этом конспекте, здесь будет преимущественно теория. Файл лежит в папке тренинга СПБГУ 2012г.**

## Оптимизации одномерного ДП

**Sliding window**

Вспомним задачу прошлую задачу которую мы разбирали. Тогда, у нас было всего 3 варианта как попасть в i позицию, а что, если у нас будет k вариантов попасть в i позицию? Напишем вложенный цикл, итого ассиптотика O(nk). Можем ускорить с помощью окна.

```cpp
#define mod 998244353//считаем все по модулю, что-бы не получить overflow
void solve() {
    int n, k;
    cin >> n >> k;

    vector<ll> dp(n + 1, 0);
    dp[0] = 1;//база
    ll window = 0;//наше окно с суммой i-k до i-1 (то есть, все возможные предки)

    for (int i = 1; i <= n; i++) {
        window = (window + dp[i - 1]) % mod;//добавляем предка
        if (i - k - 1 >= 0) window = (window - dp[i - k - 1] + mod) % mod;//если самый первый предок вышел за диапазон, вычитаем его из окна, тк сумма может оказаться < 0, прибавляем mod.
        dp[i] = window;//фиксируем ДП
    }

    cout << dp[n] << "\n";//итого O(n) вместо O(nk) 
}
```
**Монотонный дек**
#### Задача

Есть `n` клеток, у клетки `i` цена `a[i]`. Стартуем в клетке 0 и платим `a[0]`. За прыжок можно перейти на 1..k клеток вперёд, и мы платим цену клетки, в которую попали. Нужна минимальная стоимость дойти до клетки `n-1`.


Значит, нам нужно как можно дешевле добраться до n-1. Получаем:

    dp[i] = a[i] + min(dp[j]) где j это предок i в диапазоне i-k до i-1;

**решение без оптимизации O(nk)**

```cpp
void solve() {
    int n, k;
    cin >> n >> k;
    vector<ll> a(n), dp(n);
    for(int i = 0; i < n; i++)cin >> a[i];
    
    dp[0] = a[0];//база
    for (int i = 1; i < n; i++) {
        ll best = lmax;//пока не нашли лучшего предка
        for (int j = max(0, i - k); j < i; j++) {
            best = min(best, dp[j]);//перебираем весь диапазон k и ищем лучшего предка
        }
        dp[i] = a[i] + best;//фиксируем ДП
    }
    cout << dp[n - 1] << "\n";
}
```
Итак, оптимизация окном тут не сработает, ведь у нас не сумма на отрезке, а поиск минимума на отрезке.

Решение: будем использовать монотонный дек который будет хранить индексы минимумов в окне по не убыванию, мы всегда знаем когда вышел текущий минимум на отрезке и какой текущий минимум.

```cpp
void solve() {
    int n, k;
    cin >> n >> k;
    vector<ll> a(n), dp(n);
    for(int i = 0; i < n; i++)cin >> a[i];

    deque<int> dq;//дек
    dp[0] = a[0];
    dq.push_back(0);//база

    for (int i = 1; i < n; i++) {
        while (dq.front() < i - k) dq.pop_front();//если минимум вышел из диапазона, удаляем
        dp[i] = a[i] + dp[dq.front()];//фиксируем ДП, текущую цену + минимум диапазона
        while (!dq.empty() && dp[dq.back()] >= dp[i]) dq.pop_back();//удаляем все элементы из дека которые меньше текущего значения, тк они пришли раньше и меньше или равны текущего, они никогда не понадобятся.
        dq.push_back(i);//добавляем новый элемент в дек
    }
    cout << dp[n - 1] << "\n";//итог O(n)
}
```

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTI5NDY3NzQ4M119
-->