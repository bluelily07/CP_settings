*cf handle: bluelily07*

> Данный конспект будет содержать много математики, будут рассматриваться математические примеры и задачи, кода будет не так много.

## Комбинаторика - наука о подсчете
Комбинаторика отвечает на вопросы типов: 


 - Сколько есть разных способов сделать что-либо?
 - Сколько объектов удовлетворяет какому-либо условию?

**Примеры:**

-   Сколько существует перестановок массива с определённым свойством?
-   Сколькими способами разбить n предметов на k групп?
-   Сколько путей в графе из точки A в точку B с определенным условием?




## Множества и операции над множествами 

Множество - совокупность некоторых элементов.

**Операции над множеством:**

 - ∪ - Объединение множеств (A ∪ B)

 ![unit_u_495](https://media.geeksforgeeks.org/wp-content/uploads/20250411172445078226/unit_u_495.webp)
 
 A = {2, 3, 4}
 
 B = {3, 4, 5}. 
 
> A ∪ B = {2, 3, 4, 5}.

Учитывает все элементы из двух множеств один раз, без повторений.

 - ∩ - Пересечение множеств (A ∩ B)
 
 ![uni495](https://media.geeksforgeeks.org/wp-content/uploads/20250411172535654131/uni495.webp)

A = {2, 3, 4}

B = {3, 4, 5}

> A ∩ B = {3, 4}.

Учитывает элементы из двух множеств, которые одновременно входят в эти два множества.

Пересечение можно вывести еще так: 

(A ∩ B) = (A) + (B) – (A ∪ B)

 - Непересекающиеся множества
 
 ![6757](https://media.geeksforgeeks.org/wp-content/uploads/20250411172707663120/6757.webp)
 
 Пересечение таких множеств равно пустому множеству.
 
 ∅ - Пустое множество.
 
 A = {1, 3, 5, 7, 9}
 
 B = {2, 4, 6, 8}

> A ∩ B = ∅

 - A - B. Разность множеств
 
 ![file](https://media.geeksforgeeks.org/wp-content/uploads/20250411172924735560/file.webp)
 
 Элементы которые входят только в А, и не входят в A ∩ B.
 
 A = {1, 2, 3, 4, 5}
 
 B = {2, 4, 6, 8}

> A - B = {1, 3, 5}


 - △ - Симметрическая разность (A △ B или A ⊕ B)
 
![t_u_495](https://media.geeksforgeeks.org/wp-content/uploads/20250411174157863278/t_u_495.webp)

A = {1, 2, 3}

B = {3, 4, 5}

Включает в себя все элементы A и B, но не включает их пересечение.

> A △ B = {1, 2, 4, 5}


 - Дополнение множества
 
 
![76](https://media.geeksforgeeks.org/wp-content/uploads/20250411173142259586/76.webp)

Дополнение множества - элементы не входящие в множество A.
Пусть дополнением множества А будет А', U - Универсальное множество.

U = {1, 2, 3, 4, 5, 6, 7, 8}

A = {1, 2, 5, 6}

тогда по иллюстрации: (A') = (U) - (A).

> A′ = {3, 4, 7, 8}

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIyMDU4NjY2OCw0OTE3NDYyMTNdfQ==
-->